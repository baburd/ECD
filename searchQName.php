...
$lbl=$_POST['lbl'];
	...
	{
  $sql="select * from appointQExpert where effYearSem='".$lbl."' and expertName like ?";
 // echo $sql;
  ...
  $stmt->execute($params);
	...
	while ($row = $stmt->fetch()) 
	{
		...
		$label = $row{'label'};
		$status = $row{'status'}==1?"<font color='GREEN'>Receipt</font>":"<font color='RED'>Click to Receipt</font>";
