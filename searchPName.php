...
include("dbconfig.php");
  ...
  $sql="select * from assignPacket where examName='".$lbl."' and expertName like '%".$commentBy."%'";
  ...
  $stmt->execute();
	...
	while ($row = $stmt->fetch()) 
	{
		...
		$dateTime = date('Y-m-d'); 
		$diffDate = abs($dateTime - $issueDate)/86400;
		
		$npReceipt='<a href="javascript:UpdatePReceipt(\''.$codeNo.'\',\''.$subCode.'\',\''.$dateTime.'\')"><font color="RED">Confirm Receipt(-'.$diffDate.' days)</font></a>';
